...
export default class SubscriberList extends Component{
  ...
  getSubscriberList = () => {
    ...
    let url = process.env.REACT_APP_BACKEND_BASE_URL + "/admin/subscribers?search_str=" + this.state.search_str + "&page=" + this.state.page
    fetch(url, {
      method: "GET",
      headers: {
  ...
  getNextPage = (current_page, total_pages) => {
    ...
    const subscriberList = this.state.subscribers.map((subscriber, index) => {
      return (
        <tr key={index}>
          <td></td>
          <td>
            <div className="user_name_box">
              <span>{subscriber.name[0].toUpperCase()}</span>
              <p>{subscriber.name }</p>
            </div>
          </td>
