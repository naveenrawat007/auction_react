...
export default class LiveBidding extends Component{
  ...
  getPropertiesList = () => {
    ...
    let url = process.env.REACT_APP_BACKEND_BASE_URL + "/admin/properties?status=Live Online Bidding&search_str=" + this.state.search_str + "&page=" + this.state.page
    fetch(url, {
      method: "GET",
      headers: {
        ...
        if (time){
          ...
          if (document.getElementById("timer"+id)){
            if (t<0){
              ...
              document.getElementById("timer"+id).innerHTML = `-${String(days)}:${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`
            }
          }
        }else {
          if (document.getElementById("timer"+id)){
