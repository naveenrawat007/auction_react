...
export default class Terminated extends Component{
  _isMounted = false
  ...
  getPropertiesList = () => {
    ...
    let url = process.env.REACT_APP_BACKEND_BASE_URL + "/admin/properties?status=Terminated&search_str=" + this.state.search_str + "&page=" + this.state.page
    fetch(url, {
      method: "GET",
      headers: {
    ...
    const propertyList = this.state.properties.map((property, index) => {
      return (
        <tr key={index}>
          ...
          <td>{window.formatFullDate(property.termination_date)}</td>
